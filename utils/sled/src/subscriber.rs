            ...
            let mut future_rx = if let Some(future_rx) = self.existing.take() {
                ...
                match self.rx.try_recv() {
                    ...
                    Err(TryRecvError::Disconnected) => {
                        // log::warn!("[sled/subscriber] poll disconnected");
...
impl Subscribers {
    ...
    pub(crate) fn reserve<R: AsRef<[u8]>>(
        ...
        for (key, subs_rwl) in prefixes {
            ...
            for (id, (waker, sender)) in subs.iter() {
                ...
                if let Err(err) = sender.try_send(rx) {
                    log::warn!("[sled/subscriber] channel: {:?}, prefix: {:?}", err, key);
        ...
        // clear the full channel
        if full_subs.len() > 0 {
            log::warn!("[sled/subscriber] clear full channel: {:?}", full_subs);
            for (id, subs) in full_subs.into_iter() {
                let mut w_subs = subs.write();
                w_subs.remove(&id);
            }
        }
