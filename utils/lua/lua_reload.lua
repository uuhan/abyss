...
local envMetatable = {
    __index = function(t, k)
        if trackGlobals then
            ...
            if debugLogEnabled and logGlobalAccess then
                Log("  get global", k, "value: [", _GOriginal[k], "]")
            end
        end
        return _GOriginal[k]
    end,
    __newindex = function(t, k, v)
        if trackGlobals then
            globalsAccessed[k] = true
            if debugLogEnabled and logGlobalAccess then
                Log("  set global", k, "from: [", _GOriginal[k], "] to: [", v, "]")
            end
        end
        _GOriginal[k] = v
    end,
...
local setfenv = setfenv
    or function(func, env)
        local i = 1
        while true do
            local name = getupvalue(func, i)
            if name == "_ENV" then
                upvaluejoin(func, i, function()
                    return env
                end, 1)
                break
...
local setfenvNew = function(target, env)
    ...
    if
        (_sourceFileIndex_current ~= nil and _sourceFileIndex_current ~= _sourceFileIndex)
        or (_sourceFileName_current ~= nil and _sourceFileName_current ~= _sourceFileName)
    then
        -- TODO add a test case for this
        error(
            "You set the environment of two functions defined in a different files to the same table.\n"
...
local function UpdateReturnValues(file, fileIndex, ...)
    ...
    for data, value in pairs(returnValues) do
        if debugLogEnabled then
            Log("Analyze return value no.", data.valueIndex, "for fileIndex", fileIndex)
        end
        if data.fileIndex == fileIndex and type(value) == "function" then
            local newValue = select(data.valueIndex, ...)
            if debugLogEnabled then
                Log("  Update this old function from", returnValues[data], "to", newValue)
            end
...
local function ScheduleReload(fileName)
    ...
    if not toBeReload[fileName] then
        if infoLogEnabled then
            Log("Schedule", fileName, "to be reloaded", trace)
        end
        toBeReload[fileName] = true
    else
        if debugLogEnabled then
            Log(fileName, "is already scheduled to be reloaded", trace)
        end
    end
end
...
local function ReloadFile(fileName)
    ...
    file.chunk = chunk
    file.timestamp = M.FileGetTimestamp(fileName)
...
local loadfileInternal = function(fileName)
    local file = fileCache[fileName]
    local errorMessage
    local timestamp = M.FileGetTimestamp(fileName)

    if file and (not enableTimestampCheck or timestamp > file.timestamp) and M.ShouldReload(fileName) then
        if reloading then
            -- schedule to reload
            ScheduleReload(fileName)
        else
    ...
    elseif file and enableTimestampCheck and useCache and timestamp == file.timestamp then
        ...
        if logCacheAccess then
            Log(" Loading", fileName, "from cache")
        end
    else
        -- load from file
        if infoLogEnabled then
...
local function requireNew(modname)
    local function DoesFileExist(fileName)
        -- a/b/c
        local fileNameTest = hotReloadPath .. "/" .. fileName .. ".lua"
        local file = io.open(fileNameTest, "r")
        if file ~= nil then
            io.close(file)
            return fileNameTest, true
        end

        -- a/b/c/init.lua
        fileNameTest = hotReloadPath .. "/" .. fileName .. "/init.lua"
        file = io.open(fileNameTest, "r")
        if file ~= nil then
            io.close(file)
            return fileNameTest, true
        else
    ...
    fileName, fileExist = DoesFileExist(fileName)
    if fileExist and not fileCache[fileName] then
        local func, msg = loadfileNew(fileName)
        assert(func, msg)
...
local function FindReferences(fileName)
    ...
    if debugLogEnabled then
        Log("[ref_search]  Searching for references started")
    end
    ...
    -- initialization
    local preallocateTable = M.PreallocateTable
    ...
    while ptr < size do
        ...
        if currentType == "table" and not visited[currentValue] then
            ...
            for k, v2 in pairs(currentValue) do
                ...
                if t == "table" or t == "function" then
                    ...
                    if storePath then
                        queuePath[size] = currentPath .. "/" .. tostring(k)
                    end
                end
                local t = type(k)
                if t == "table" or t == "function" then
        ...
        elseif currentType == "function" then
            ...
            if not visited[currentValue] then
                if traverseEnvs and env ~= _GOriginal and env ~= envMetatable then
                    ...
                    queueName[size] = "ENV"
                    if storePath then
                        queuePath[size] = currentPath .. "/" .. queueName[size]
                    end
                end
                ...
                while true do
                    ...
                    if ln ~= nil then
                        ...
                        if t == "table" or t == "function" then
                            ...
                            if storePath then
                                queuePath[size] = currentPath .. "/" .. tostring(ln)
                            end
                        end
                    else
                        break
        ...
        if debugLogEnabled and ptr % 100000 == 0 then
            Log("[ref_search] did ", ptr, " steps")
        end
    end

    if debugLogEnabled then
        Log("[ref_search]  Finished in", ptr, "steps")
    end
...
local function traverse(data, fileName, visitedDuringSearch)
    if debugLogEnabled then
        Log("-> traverse", fileName)
    end
    local storePath = debugLogEnabled
    local logContent = debugLogEnabled and false
    local logPush = debugLogEnabled and false
    ...
    local function push(obj, name, previous, link)
        ...
        queueLink[size] = link
        if storePath then
            queuePath[size] = (queuePath[previous] or "") .. "/[" .. tostring(name) .. "]"
        end
        if logPush then
            Log("  push [", obj, "] at", size)
        end
    ...
    while ptr < size do
        ...
        local currentType = type(currentValue)
        if storePath then
            Log(ptr .. ".", queuePath[ptr], "= [", currentValue, "]")
        end
        ...
        if currentType == "function" then
            ...
            if not visitedDuringSearch[currentValue] or (useGetInfo and not getfenv(currentValue)) then
                local target = false
                local env = getfenv(currentValue)
                if useGetInfo then
                    local info = functionSource[currentValue]
                        ...
                        while true do
                            ...
                            if ln ~= nil then
                                ...
                                if upvalues[ln] and upvalues[ln].id ~= upvalueid then
                                    Error(
                                        "Two different upvalues with the same name found, please rename one of them. Upvalue `"
                                        .. tostring(ln)
                                        .. "` referenced in",
                                        GetFuncDesc(upvalues[ln].func),
                                        "and",
                                        GetFuncDesc(currentValue)
                                    )
                                end
                                if logContent then
                                    Log("  - upvalue [", ln, "] = [", lv, "]")
    ...
    if debugLogEnabled then
        Log("traverse completed in", size, "steps")
    end

    if debugLogEnabled and inspect then
        Log("Upvalues data:")
...
local function SeparateReferencesByUpvalues(references, returnValuesByIndex)
    ...
    if debugLogEnabled then
        Log("Creating a map [function : fileIndex] based on captured return values")
    end
    for index, _returnValues in pairs(returnValuesByIndex) do
        if not references[index] then
            if debugLogEnabled then
                Log("  Traverse return values for fileIndex no.", index, "in order to look for functions")
            end
            ...
            while ptr < size do
                ...
                elseif vtype == "function" then
                    if debugLogEnabled then
                        Log("   ", GetFuncDesc(value), "is associated with fileIndex", index)
                    end
    ...
    if debugLogEnabled then
        Log("Separating functions into buckets based on their upvalues")
    end
    ...
    for fileIndex, list in pairs(references) do
        for _, ref in ipairs(list) do
            ...
            if debugLogEnabled then
                Log(" ", GetFuncDesc(func), "is associated with file index", fileIndex)
            end
    ...
    for _, ref in ipairs(refList) do
        ...
        if debugLogEnabled then
            Log("  Analyzing", GetFuncDesc(func))
        end
        ...
        if myIndex then
            ...
            if debugLogEnabled then
                Log("    This function's fileIndex was deduced based on captured return values, and it is", myIndex)
            end
        else
            ...
            while true do
                ...
                if upvalueidToFunc[id] then
                    ...
                    myIndexes[myIndex] = true
                    if debugLogEnabled then
                        Log("    Upvalue with id", id, "is referred to the bucket no.", myIndex)
                    end
                else
                    upvalueidToFunc[id] = func
                    if debugLogEnabled then
                        Log("    Upvalue with id", id, "seems to be new")
                    end
                end
                i = i + 1
            end
            ...
            if myFirstIndex == nil then
                ...
                myIndex = index
                if debugLogEnabled then
                    Log("    All upvalues were new, introducing a new bucket with id", myIndex)
                end
            else
                myIndex = myFirstIndex
                if debugLogEnabled then
                    Log("    This function shared upvalues with functions in the bucket no.", myIndex)
                end
                for i, _ in pairs(myIndexes) do
                    if i ~= myIndex then
                        ...
                        if debugLogEnabled then
                            Log(
                                "    Moving all functions from bucket no.",
                                i,
                                "to above bucket,",
                                "since those functions are sharing upvalues"
                            )
                        end
                        references[i] = nil
                    end
        ...
        if currentType == "function" then
            ...
            if step.upvalueName then
                ...
                if not found then
                    if debugLogEnabled then
                        Log(
                            "  wasn't able to take step no.",
                            #route - stepNumber + 1,
                            "in the new file, missing upvalue",
                            step.upvalueName,
                            "(originally at index",
                            tostring(step.upvalueIndex) .. ")"
                        )
                    end
                    newValueFound = false
                    break
...
Reload = function(fileName, chunkOriginal, chunkNew, returnValues)
    ...
    if debugLogEnabled then
        Log("\n*** PREPARE RETURN VALUES BY INDEX  ***")
    end
    ...
    for k, v in pairs(returnValues) do
        ...
        t[k.valueIndex] = v
        if debugLogEnabled then
            Log("return value for file with index [", k.fileIndex, "] no.", k.valueIndex, "is", v)
        end
    end
    ...
    for index, _ in pairs(returnValuesByIndex) do
        if not references[index] then
            references[index] = {}
            if debugLogEnabled then
                Log("adding empty reference list based on return values", "for file index", index)
            end
        end
    end
    ...
    if debugLogEnabled then
        Log("FOUND", versions, "VERSIONS OF THE", fileName, ":")
    end
    ...
    if debugLogEnabled then
        Log("\n*** BUILDING ROUTES TO FUNCTIONS IN ORIGINAL FILE ***")
    end

    globalsAccessed = {}
    if handleGlobalModules then
        if debugLogEnabled then
            Log("tracking globals...")
        end
        trackGlobals = true
    ...
    for name, _ in pairs(globalsAccessed) do
        ...
        if debugLogEnabled then
            Log("  schedule global [", name, "] to be traversed")
        end
    end
    ...
    for fileIndex, list in pairs(references) do
        if debugLogEnabled then
            Log("\n*** UPDATING FILE WITH INDEX", fileIndex, "***")
        end
        ...
        if debugLogEnabled then
            Log("\n*** UPDATING REFERENCES && SET ENVS OF THE NEW FUNCs TO ENV OF OLD ONES ***")
        end
        if debugLogEnabled and #list == 0 then
            Log("There are no references for this file index - no functions to update.")
        end
            ...
            if not functionPtrList then
                if debugLogEnabled then
                    Log(
                        refIndex .. ".",
                        ref.value,
                        "- wasn't able to find this one, probably it's a function made in another function"
                        .. "\n  It was defined at line",
                        linedefined
                    )
                end
            else
                for routeIndex, routeStart in ipairs(functionPtrList) do
                    if debugLogEnabled then
                        Log(
                            ...
                            ref.value,
                            "found in",
                            fileData1.queuePath and fileData1.queuePath[routeStart]
                            or "<path of reference isn't stored>",
                            "(route#" .. routeIndex .. ")"
                        )
                    end
                    ...
                    if not newValueFound then
                        if debugLogEnabled then
                            Log("  wasn't able to retrieve new value")
                        end
                    else
                        if debugLogEnabled then
                            Log("  found new value", currentValue)
                        end
                        ...
                        if currentValue == nil and routeIndex ~= #functionPtrList then
                            if debugLogEnabled then
                                Log(
                                    "  but is is a nil value, so first we should check other routes - maybe we will find non-nil value"
                                )
                            end
                        else
                            ...
                            elseif type(ref.link.owner) == "table" then
                                    ...
                                    if currentValue then
                                        ...
                                        if debugLogEnabled then
                                            Log(
                                                ...
                                                ref.link.owner,
                                                "to [",
                                                ref.link.owner[ref.value],
                                                "]"
                                            )
                                        end
                                    else
                                        if debugLogEnabled then
                                            Log("  remove key [", ref.value, "] in", ref.link.owner)
                                        end
                                    end
                                ...
                                error(
                                    "DEV ERROR: invalid owner type in the reference, probably code gathering references is broken"
                                )
                            end
        ...
        if debugLogEnabled then
            Log("\n*** BUILDING CURRENT UPVALUES LIST ***")
        end
            ...
            if not visited[linedefined] then
                ...
                if isFileScopeFunction or retrieveUpvaluesFromNestedFunctions then
                            ...
                            if upvaluesOriginal[ln] then
                                ...
                                if upvalues[ln] and upvalues[ln].id ~= upvalueid then
                                    ...
                                    if not isFileScopeFunction and not isFileScopeFunction_old then
                                        if debugLogEnabled then
                                            Log(
                                                "Note: Two different upvalues with the same names, referenced in two different nested functions\n",
                                                "with name [",
                                                ln,
                                                "] will be ignored for safety. Because they probably aren't file-scoped upvalues.\n",
                                    ...
                                    elseif isFileScopeFunction and isFileScopeFunction_old then
                                        error(
                                            "DEV ERROR: something went wrong... Two different file-scoped functions with two different upvalues with the same name?.\n"
                                        ...
                                        if debugLogEnabled then
                                            Log(
                                                "Note: There are at least two upvalues with the same name [",
                                                ln,
                                                "],\n",
                    ...
                    if debugLogEnabled then
                        Log(
                            GetFuncDesc(currentValue),
                            "is seems to be a nested function, so we ignore its upvalues.\n",
        ...
        for k in pairs(ignoreUpvalues) do
            ...
            if debugLogEnabled then
                Log(
                    "Note: removing upvalue [",
                    k,
                    "] from the list, because we aren't sure if it's a file-scoped upvalue or not."
                )
            end
        end
        ...
        if debugLogEnabled then
            Log("\n*** UPDATING OLD CONST UPVALUES TO NEW VALUES ***")
        end
        ...
        for k, _ in pairs(upvaluesCurrent) do
            if upvaluesOriginal[k] then
                if not upvaluesNew[k] then
                    if debugLogEnabled then
                        Log("  Note: upvalue [", k, "] seems to be removed from the new version of the file.")
                    end
                end
                push({
                ...
                if debugLogEnabled then
                    Log(
                        "  Note: upvalue [",
                        k,
                        "] isn't present in the original file, so it is probably an upvalue of a nested function."
                    )
                end
            end
        end
        ...
        if debugLogEnabled then
            Log("  Current fileIndex is", fileIndex)
        end
        ...
        if returnedTables then
            for i, v in pairs(returnedTables) do
                ...
                if type(newValue) == "table" then
                    ...
                    if debugLogEnabled and type(v) == "table" then
                        Log(
                            "  Note: return value no. [",
                            i,
                            "] from the original file, which was a table, is",
                            type(newValue)
                        )
                    end
                end
            ...
            if debugLogEnabled then
                Log(
                    "  Note: old version of the file didn't return any tables, no special handling of the returned values is required."
                )
            end
        end
        ...
        while ptr < size do
            ...
            if debugLogEnabled then
                Log(
                    ptr .. ". stored in: [",
                    queueLink[ptr].key or queueLink[ptr].debugName,
                    "] current: [",
                    obj.current,
                    "] original: [",
                    obj.original,
            ...
            if obj.current ~= obj.new then
                ...
                if currentType == "table" and type(obj.original) == "table" and type(obj.new) == "table" then
                    if not visited[obj.current] then
                        ...
                        if not isArray or isStatic then
                            if debugLogEnabled then
                                Log(
                                    "  it's a table value, and we are gonna traverse it further either because it's a dynamic array or not an array at all."
                                )
                            end
                            -- traverse further
                            ...
                            if debugLogEnabled then
                                Log(
                                    "  it's a table value, but we aren't gonna traverse it, because it's a static array."
                                )
                            end
                        end
                    end
                elseif currentType ~= "function " and currentType ~= "thread" and currentType ~= "userdata" then
                    ...
                    if isTableToPOD then
                        if debugLogEnabled then
                            Log(
                                "  it's a table, which is a POD value in the new version of the file\n"
                                ..
                                "  So we just change this value into a new POD, but if there are anonymous functions from old file using this value, they may broke."
                            )
                        end
                    end
                    ...
                    if isConst or isTableToPOD then
                        ...
                        elseif ref.table then
                            ...
                            error(
                                "DEV ERROR: we try to update a value which isn't stored anywhere, either this the link is broken, or we are updating a return value."
                            )
                        end
                    else
                        if debugLogEnabled then
                            Log("  it's NOT a CONST value")
                        end
                    end
                else
                    if debugLogEnabled then
                        Log("  it's a", currentType, "value - ignored")
                    end
                end
            end
        end
        ...
        if debugLogEnabled then
            Log("\n*** MERGING UPVALUES OF NEW FUNCTIONS TO OLD ONES ***")
        end
        for _, ptrList in pairs(fileData2.functions) do
            local ptr = ptrList[1]
            local func = fileData2.queue[ptr]
            if debugLogEnabled then
                Log("- processing function", func, "in ptr", ptr)
            end
            local i = 1
            while true do
                    ...
                    if upvalue_new then
                        ...
                        if debugLogEnabled then
                            Log(
                                ...
                                lv,
                                "] now [",
                                select(2, getupvalue(upvalue_new.func, upvalue_new.index)),
                                "])"
                            )
                        end
                    else
                        if debugLogEnabled then
                            Log("  wasn't able to find upvalue [", ln, "] in the old version of the file")
                        end
                        ...
                        if newValue then
                            ...
                            if debugLogEnabled then
                                Log(
                                    ...
                                    "] to table [",
                                    newValue,
                                    "] which was found during comparison of the data beetwen the original and new files"
                                )
                            end
                        end
                    end
                else
                    if debugLogEnabled and i == 1 then
                        Log("  no upvalues")
                    end
                    break
                end
...
function M.SetHotReloadPath(path)
    hotReloadPath = path
end

function M.SetPrintInfoLogs(enable)
    infoLogEnabled = enable
end

function M.SetPrintDebugLogs(enable)
    debugLogEnabled = enable
end

function M.SetLogCacheAccess(enable)
    logCacheAccess = enable
end

function M.SetLogReferencesSteps(enable)
    logReferencesSteps = enable
end

function M.SetStoreReferencePath(enable)
    storeReferencePath = enable
end

function M.SetLogUpvalues(enable)
    logUpvalues = enable
end

function M.SetTraverseGlobals(enable)
    traverseGlobals = enable
end

function M.SetTraverseRegistry(enable)
    traverseRegistry = enable
end

function M.SetHandleGlobalModules(enable)
...
function M.GetFileCache()
    return fileCache
end

function M.ClearFileCache()
    fileCache = {}
end

function M.SetUseCache(enable)
    useCache = enable
end

function M.SetUseOldFileOnError(enable)
    useOldFileOnError = enable
end

function M.SetUseGetInfo(enable)
    useGetInfo = enable
end

function M.SetQueuePreallocationSize(value)
    queuePreallocationSize = value
end

function M.SetVisitedPreallocationSize(value)
    visitedPreallocationSize = value
end

-- a game is expected to provide getTimestamp function
local staticTimestamp = 0
function M.FileGetTimestamp(fileName)
    if lfs then
...
function M.SetEnableTimestampCheck(enable)
...
local monitorPtr = 1
function M.Monitor(step, debugLogEnabled)
        ...
        if cached then
            ...
            if timestamp and timestamp > cached.timestamp then
                ...
                if file then
                    ...
                    if success then
                        if debugLogEnabled then
                            Log("Reloading", filename, "old timestamp:", cached.timestamp, "new timestamp:", timestamp)
                        end
                        ScheduleReload(filename)
                        reloading = true
                    else
                        if debugLogEnabled then
                            Log("Failed to retrieve lock on a file")
                        end
                    end
                end
            end
...
function M.ShouldReload(fileName)
    return true
end

function M.ReloadFile(fileName, ignoreTimestamp)
    ...
    local file = fileCache[fileName]
    if file and M.ShouldReload(fileName) then
        local timestamp = M.FileGetTimestamp(fileName)
...
function M.ReloadScheduledFiles()
    ReloadScheduledFiles()
end

function M.SetVisited(k, v)
    visitedGlobal[k] = v
end
