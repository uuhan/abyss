...
#[cfg(feature = "measure_allocs")]
#[global_allocator]
static ALLOCATOR: measure_allocs::TrackingAllocator = measure_allocs::TrackingAllocator;
...
impl<PageFrag> From<&Update<PageFrag>> for LogKind
where
    PageFrag: DeserializeOwned + Serialize,
{
    fn from(update: &Update<PageFrag>) -> Self {
        use Update::*;
...
impl From<MessageKind> for LogKind {
    fn from(kind: MessageKind) -> Self {
        ...
        match kind {
            ...
            InlineReplace | Counter | BlobReplace | InlineMeta | BlobMeta | InlineConfig
            | BlobConfig => LogKind::Replace,
...
pub use crossbeam_epoch::{pin, unprotected, Atomic, Collector, Guard, LocalHandle, Owned, Shared};
