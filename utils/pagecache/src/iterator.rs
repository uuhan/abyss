    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            let remaining_seg_too_small_for_msg =
                !valid_entry_offset(self.cur_lsn as LogId, self.config.io_buf_size);
            ...
            match f.read_message(lid, self.cur_lsn, &self.config) {
                ...
                Ok(LogRead::Failed(_, on_disk_len)) => {
                    ...
                    self.cur_lsn += Lsn::from(MSG_HEADER_LEN as u32 + on_disk_len);
                }
...
fn scan_segment_lsns(min: Lsn, config: &Config) -> Result<(BTreeMap<Lsn, LogId>, Lsn)> {
    fn fetch(idx: u64, min: Lsn, config: &Config) -> Option<(LogId, SegmentHeader)> {
    ...
    let header_promises: Vec<Promise<Option<(LogId, SegmentHeader)>>> = (0..segments)
        .map({
            ...
            move |idx| {
                Promise::new({
                    let config = config.clone();
                    move |promise| promise.resolve(fetch(idx, min, &config))
                })
            }
        })
        .collect();
    ...
    for (lid, header) in headers {
        max_header_stable_lsn = std::cmp::max(header.max_stable_lsn, max_header_stable_lsn);
    ...
    let ordering = clean_tail_tears(max_header_stable_lsn, ordering, &config, &f)?;
...
fn clean_tail_tears(
    ...
    for (lsn, lid) in ordering.range((std::ops::Bound::Excluded(tip.0), std::ops::Bound::Unbounded))
    {
        ...
        f.pwrite_all(&*vec![MessageKind::Corrupted.into(); SEG_HEADER_LEN], *lid)?;
        if !config.temporary {
            f.sync_all()?;
        }
    }
...
pub(super) fn raw_segment_iter_from(lsn: Lsn, config: &Config) -> Result<(LogIter, Lsn)> {
    ...
    let tip_segment_iter = Box::new(ordering.iter().map(|(a, b)| (*a, *b)).last().into_iter());
    trace!(
