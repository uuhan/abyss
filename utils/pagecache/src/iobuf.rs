use abyss_promise::Task;

use self::reader::LogReader;

use super::*;
use crate::{atomic::*, sync::*};
...
impl IoBufs {
    pub(crate) fn start(config: Config, snapshot: Snapshot) -> Result<Self> {
        ...
        let (next_lsn, next_lid) = if snapshot_last_lsn % config.io_buf_size as Lsn == 0 {
            (snapshot_last_lsn, snapshot_last_lid)
        } else {
            let width = match file.read_message(snapshot_last_lid, snapshot_last_lsn, &config) {
                Ok(LogRead::Failed(_, len)) | Ok(LogRead::Inline(_, _, len)) => {
        ...
        Ok(Self {
            ...
            max_header_stable_lsn: Arc::new(AtomicLsn::new(snapshot_max_header_stable_lsn)),
    ...
    pub(crate) fn iter_from(&self, lsn: Lsn) -> LogIter {
        ...
        let segment_iter = self.with_sa(|sa| sa.segment_snapshot_iter_from(corrected_lsn));
    ...
    pub(crate) fn write_to_log(&self, iobuf: &IoBuf) -> Result<()> {
        ...
        if should_pad {
            ...
            unsafe {
                std::ptr::copy_nonoverlapping(
                    crc32_arr.as_ptr(),
                    data.as_mut_ptr()
                        .add(bytes_to_write + MSG_HEADER_LEN - std::mem::size_of::<u32>()),
                    std::mem::size_of::<u32>(),
                );
            }
        }
        ...
        let current_max_header_stable_lsn = self.max_header_stable_lsn.load(SeqCst);

        // TODO make SA lock-free so we don't have to defer this occasionally
        if let Some(ret) = self.try_with_sa(|sa| sa.stabilize(current_max_header_stable_lsn)) {
            ret
        } else {
            Ok(())
        }
...
pub(crate) fn maybe_seal_and_write_iobuf(
    ...
    *mu = Arc::new(next_iobuf);
    ...
    if n_writers(sealed) == 0 {
        ...
        let _result = Task::run(move || {
            if let Err(e) = iobufs.write_to_log(&iobuf) {
                error!("hit error while writing iobuf with lsn {}: {:?}", lsn, e);
                let _lock = iobufs.intervals.lock();
                iobufs.interval_updated.notify_all();
...
impl IoBuf {
    ...
    pub(crate) fn store_segment_header(&mut self, last: Header, lsn: Lsn, max_stable_lsn: Lsn) {
