...
const DEFAULT_PATH: &str = "magnet.db";
...
impl Config {
    ...
    pub fn verify_snapshot(&self) -> Result<()> {
        ...
        let verify_pagestate =
            |x: &FastMap8<PageId, PageState>, y: &FastMap8<PageId, PageState>, typ: &str| {
                for (k, v) in x {
                    if !y.contains_key(&k) {
...
fn get_cgroup_memory_limit() -> io::Result<u64> {
    File::open("/sys/fs/cgroup/memory/memory.limit_in_bytes").and_then(read_u64_from)
}
