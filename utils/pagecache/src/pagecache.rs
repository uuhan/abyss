...
impl<'a> RecoveryGuard<'a> {
    ...
    pub fn seal_batch(mut self) -> Result<()> {
        let max_reserved = self.batch_res.log.iobufs.max_reserved_lsn.load(Acquire);
...
/// The core data structure for stable storage.
#[derive(Clone)]
pub struct PageCache<P: Materializer>(Arc<PageCacheInner<P>>);

impl<P> PageCache<P>
where
    P: Materializer,
{
    /// Start some necessary facility.
    pub fn start(config: Config) -> Result<Self> {
        let inner = PageCacheInner::start(config)?;
...
impl<P: Materializer> Deref for PageCache<P> {
    type Target = PageCacheInner<P>;

    fn deref(&self) -> &PageCacheInner<P> {
        &self.0
    }
}
...
unsafe impl<P> Send for PageCacheInner<P> where P: Materializer {}
unsafe impl<P> Sync for PageCacheInner<P> where P: Materializer {}

/// A lock-free pagecache which supports fragmented pages
/// for dramatically improving write throughput.
pub struct PageCacheInner<P>
where
    P: Materializer,
{
    config: Config,
    ...
    free: Arc<Mutex<BinaryHeap<PageId, MaxComparator>>>,
    log: Log,
    lru: Lru,
    updates: AtomicU64,
    ...
    P: Materializer,
{
    ...
    pub fn start(config: Config) -> Result<Self> {
...

        {
            ...
            if let Err(Error::ReportableBug(..)) = pc.get_persisted_config(&guard) {
                // set up idgen
                was_recovered = false;

                let config_update = Update::Config(PersistedConfig);

                let (config_id, _) = pc.allocate_inner(config_update, &guard)?;
            ...
            let idgen_persists =
                counter / pc.config.idgen_persist_interval * pc.config.idgen_persist_interval;
    ...
    /// and `link` operations.
    pub fn allocate<'g>(&self, new: P, guard: &'g Guard) -> Result<(PageId, PagePtr<'g, P>)> {
    ...
    pub fn set_failpoint(&self, e: Error) {
        if let Error::FailPoint = e {
            ...
            let _lock = self.log.iobufs.intervals.lock();
    ...
    pub fn free<'g>(
        ...
        if pid == COUNTER_PID || pid == META_PID || pid == CONFIG_PID || pid == BATCH_MANIFEST_PID {
    ...
    pub fn link<'g>(
        ...
        if stack_len >= self.config.page_consolidation_threshold {
            let current_frag = if let Some((current_ptr, frag, _sz)) = self.get(pid, guard)? {
                if old.ts != current_ptr.ts && old.cached_ptr != current_ptr.cached_ptr {
        ...
        loop {
            ...
            if let (Some(Update::Append(_)), ref mut stored_cache_info) = node.inner {
                *stored_cache_info = cache_info;
            } else {
                panic!("should only be working with Resident entries");
            }

            let result = unsafe { head_ptr.deref().cap_node(old.cached_ptr, node, &guard) };
            ...
            match result {
                Ok(cached_ptr) => {
                    ...
                    let previous_lsn_segment = previous_head_lsn / self.config.io_buf_size as i64;
    ...
    pub fn replace<'g>(
        ...
        let result = self.cas_page(pid, old, Update::Compact(new), false, guard)?;
    ...
    fn cas_page<'g>(
        ...
        trace!(
            "[cas/page] called on pid {} to {:?} with old ts {:?}",
            pid,
            update,
            old.ts
        );
        ...
        loop {
            ...
            let node = node_from_frag_vec(vec![(Some(update_opt.take().unwrap()), cache_info)]);

            let result = unsafe { head_ptr.deref().cas(old.cached_ptr, node, &guard) };

            match result {
                Ok(cached_ptr) => {
                    trace!("[cas/page] succeeded on pid {}", pid);
                    let pointers = ptrs_from_stack(old.cached_ptr, guard);

                    self.log
                        .with_sa(|sa| sa.mark_replace(pid, lsn, pointers, new_ptr))?;
                ...
                Err((actual_ptr, returned_entry)) => {
                    trace!("[cas/page] failed on pid {}", pid);
                    log_reservation.abort()?;

                    let returned_update = returned_entry.into_box().inner.0.take().unwrap();
                    ...
                    trace!("retrying CAS on pid {} with same ts of {}", pid, old.ts);
                    old = PagePtr {
                        cached_ptr: actual_ptr,
                        ts: old.ts,
                    };
    ...
    pub(crate) fn get_meta<'g>(&self, guard: &'g Guard) -> Result<(PagePtr<'g, P>, &'g Meta)> {
        ...
        match StackIter::from_ptr(head, &guard).next() {
            ...
            Some((None, cache_info)) => {
                let update = self.pull(META_PID, cache_info.lsn, cache_info.ptr)?;
                log::trace!("[pull/meta] {:?}", update);
                let ptr = PagePtr {
                    cached_ptr: head,
                    ts: cache_info.ts,
                };
    ...
    pub(crate) fn get_persisted_config<'g>(
        ...
        match StackIter::from_ptr(head, &guard).next() {
            ...
            Some((None, cache_info)) => {
                let update = self.pull(CONFIG_PID, cache_info.lsn, cache_info.ptr)?;
                let ptr = PagePtr {
                    cached_ptr: head,
                    ts: cache_info.ts,
                };
    ...
    pub(crate) fn get_idgen<'g>(&self, guard: &'g Guard) -> Result<(PagePtr<'g, P>, u64)> {
        ...
        match StackIter::from_ptr(head, &guard).next() {
            ...
            Some((None, cache_info)) => {
                let update = self.pull(COUNTER_PID, cache_info.lsn, cache_info.ptr)?;
                let ptr = PagePtr {
                    cached_ptr: head,
                    ts: cache_info.ts,
                };
                let _ = self.cas_page(COUNTER_PID, ptr, update, false, guard)?;
    ...
    pub fn get<'g>(
        ...
        let initial_base = match entries[0] {
            ...
            (Some(Update::Append(_)), _) => {
                ...
                let base_idx = entries
                    .iter()
                    .position(|(e, _)| e.is_some() && e.as_ref().unwrap().is_compact());
                if let Some(base_idx) = base_idx {
                    let mut base = entries[base_idx].0.as_ref().unwrap().as_frag().clone();
        ...
        let base = if let Some(initial_base) = initial_base {
            ...
            let pulled = entries.iter().map(|entry| match entry {
                (Some(Update::Compact(compact)), _) | (Some(Update::Append(compact)), _) => {
    ...
    pub fn generate_id(&self) -> Result<u64> {
        ...
        while persisted < necessary_persists {
            ...
            if persisted < necessary_persists {
                ...
                if self
                    .cas_page(COUNTER_PID, key.clone(), counter_update, false, &guard)?
                    .is_err()
                {
                    // CAS failed
                    continue;
    ...
    pub fn meta_pid_for_bucket(&self, name: &[u8], guard: &Guard) -> Result<PageId> {
        let m = self.meta(guard)?;
        if let Some(root) = m.get_bucket(name) {
            Ok(root)
        } else {
    ...
    /// Same as `meta_pid_for_bucket`, but for blocks
    pub fn meta_pid_for_block(&self, name: &[u8], guard: &Guard) -> Result<PageId> {
        let m = self.meta(guard)?;
        if let Some(root) = m.get_block(name) {
            Ok(root)
        } else {
            Err(Error::CollectionNotFound(name.to_vec()))
        }
    ...
    pub fn cas_block_in_meta<'g>(
        ...
        loop {
            ...
            let actual = meta.get_block(&name);
            if actual != old {
            ...
            if let Some(new) = new {
                new_meta.set_block(name.to_vec(), new);
            } else {
                new_meta.del_block(&name);
            }

            let new_meta_frag = Update::Meta(new_meta);

            let res = self.cas_page(META_PID, meta_key.clone(), new_meta_frag, false, &guard)?;
    ...
    fn pull(&self, pid: PageId, lsn: Lsn, ptr: DiskPtr) -> Result<Update<P>> {
        ...
        log::trace!("pid: {}, header: {:?}, bytes: {:?}", pid, header, bytes);
        ...
        let update_res = match header.kind {
            ...
            BlobMeta | InlineMeta => deserialize::<Meta>(&bytes).map(Update::Meta),
            BlobConfig | InlineConfig => deserialize::<PersistedConfig>(&bytes).map(Update::Config),
            BlobAppend | InlineAppend => deserialize::<P>(&bytes).map(Update::Append),
            BlobReplace | InlineReplace => deserialize::<P>(&bytes).map(Update::Compact),
            Free => Ok(Update::Free),
    ...
    fn advance_snapshot(&self) -> Result<()> {
        ...
        let config = self.config.clone();
        let _result = Promise::new(move |promise| {
            let result = gen_snapshot();
            match &result {
                Ok(_) => {}
                Err(Error::Io(ref ioe)) if ioe.kind() == std::io::ErrorKind::NotFound => {}
                Err(error) => {
                    error!("encountered error while generating snapshot: {:?}", error,);
...
fn ptrs_from_stack<'g, P>(head_ptr: PagePtrInner<'g, P>, guard: &'g Guard) -> Vec<DiskPtr>
where
    P: Materializer,
{
