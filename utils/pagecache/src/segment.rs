...
#[derive(Debug, Copy, Eq, Hash, Ord, PartialOrd, PartialEq, Clone, Serialize, Deserialize)]
...
impl Segment {
    fn len(&self) -> usize {
        std::cmp::max(self.present.len(), self.removed.len()) - self.removed.len()
    }
    ...
    fn active_to_inactive(
        ...
        let deferred_rm_blob = mem::replace(&mut self.deferred_rm_blob, FastSet8::default());
    ...
    fn defer_replace_pids(&mut self, deferred: FastSet8<(PageId, usize)>, lsn: Lsn) {
        assert!(lsn >= self.lsn.unwrap());
        self.deferred_replacements.extend(deferred);
    }

    fn remove_blob(&mut self, blob_ptr: BlobPointer, config: &Config) -> Result<()> {
        match self.state {
            Active => {
                // we have received a removal before
...
impl SegmentAccountant {
    ...
    fn initialize_from_snapshot(&mut self, snapshot: Snapshot) -> Result<()> {
        ...
        let number_of_segments = usize::try_from(file_len / io_buf_size as u64).unwrap()
            + if empty_snapshot
        ...
        let mut add = |pid, lsn, sz, lid: LogId, segments: &mut Vec<Segment>| {
        ...
        let currently_active_segment = {
            ...
            if let Some(segment) = segments.get(prospective_currently_active_segment) {
                if segment.is_empty() {
        ...
        let cleanup_threshold = (self.config.segment_cleanup_threshold * 100.) as usize;
        ...
        for (idx, segment) in segments.iter_mut().enumerate() {
            ...
            if segment_base >= self.tip {
                ...
                trace!("raised self.tip to {} during SA initialization", self.tip);
            }
            ...
            if idx != currently_active_segment
                && segment_lsn + io_buf_size as Lsn <= snapshot.max_header_stable_lsn
            {
    ...
    pub(super) fn mark_replace(
        ...
        let new_segment_start = new_idx as LogId * self.config.io_buf_size as LogId;
        ...
        for old_ptr in old_ptrs {
            ...
            if schedule_rm_blob && old_ptr.is_blob() {
                trace!("queueing blob removal for {} in our own segment", old_ptr);
                self.segments[new_idx].remove_blob(old_ptr.blob().1, &self.config)?;
            }
    ...
    pub(super) fn mark_link(&mut self, pid: PageId, lsn: Lsn, ptr: DiskPtr) {
        ...
        let segment_lsn = lsn / self.config.io_buf_size as Lsn * self.config.io_buf_size as Lsn;
    ...
    pub(super) fn stabilize(&mut self, stable_lsn: Lsn) -> Result<()> {
        ...
        if self.deferred_free_segments.is_some() && lsn > self.deferred_free_segments_after {
            let deferred_free_segments = self.deferred_free_segments.take().unwrap();
    ...
    fn deactivate_segment(&mut self, lsn: Lsn) -> Result<()> {
                ...
                self.segments
                    ...
                    .filter_map(|(i, s)| {
                        if s.present.contains(&pid) {
                            Some((i * self.config.io_buf_size, s.state, s.present.clone()))
                        } else {
                            None
                        }
                    })
        ...
        let inactive_segs = self.segments.iter().filter(|s| s.is_inactive()).count();
    ...
    pub(super) fn next(&mut self, lsn: Lsn) -> Result<LogId> {
        ...
        let free: Vec<LogId> = self
            ...
            .iter()
            .filter(|lid| {
                let idx = usize::try_from(*lid / self.config.io_buf_size as LogId).unwrap();
    ...
    fn truncate(&mut self, at: LogId) -> Result<()> {
        ...
        let result = Promise::new(move |promise| {
        ...
        self.async_truncations.push(result);
...
fn segment_is_drainable(
    ...
    let segment_low_count = len < MINIMUM_ITEMS_PER_SEGMENT * 100 / cleanup_threshold;
